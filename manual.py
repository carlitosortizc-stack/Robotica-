...
at_detector_tags = Detector(families="tag36h11")
modo_apriltag = False
tiempo_ultimo_tag = 0
LIMITE_TIEMPO_TAG = 3.0  # Segundos de espera antes de desactivar
ancho, alto = 960, 720
kp_tags = 0.1
...
while True:
    ...
    # etiqueta = "MODO TAG ACTIVO"
    if not modo_apriltag:
        # --- MODO GESTOS ---
        ...
        if etiqueta == "PAZ":
            modo_apriltag = True
            tiempo_ultimo_tag = time.time()
            print(">>> MODO APRILTAG ACTIVADO - Ignorando manos")
    
    else:
        # --- MODO APRILTAG (Ignora las manos aquí) ---
        print(">>> MODO APRILTAG ACTIVADO - Ignorando manos")

        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        tags = at_detector_tags.detect(gray)
        ...
        if tags:
            tag = tags[0]
            tag_found = True
            tiempo_ultimo_tag = time.time() # Reset temporizador
            
            # Dibujar en el frame espejado (corrigiendo X)
            tx = ancho - int(tag.center[0])
            ty = int(tag.center[1])
            cv2.drawMarker(frame, (tx, ty), (0, 0, 255), cv2.MARKER_CROSS, 30, 2)
            
            # Calcular errores de seguimiento
            # error_x = int(tag.center[0]) - (ancho // 2)
            # error_y = int(tag.center[1]) - (alto // 2)
            # yaw = int(error_x * kp_tags)
            # ud = int(-error_y * kp_tags)
            ...
            mitad = (ancho // 2)
            if tag.center[0] < mitad:
                lr = -20
            elif tag.center[0] > mitad:
                lr = 20


            cv2.rectangle(frame, (ancho//2-110, alto-80), (ancho//2+110, alto-30), (0, 150, 0), -1)
            color_ui = (0, 255, 0) if modo_apriltag else (0, 255, 255)
            cv2.putText(frame, "SIGUIENDO", (ancho//2-90, alto-45), cv2.FONT_HERSHEY_TRIPLEX, 1, (0, 255, 0), 2)
        
        else:
            # Lógica de tiempo fuera
            tiempo_pasado = time.time() - tiempo_ultimo_tag
            restante = max(0, LIMITE_TIEMPO_TAG - tiempo_pasado)
            
            cv2.putText(frame, f"SIN TAG: Volviendo en {restante:.1f}s", (50, alto-50), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
            
            if tiempo_pasado > LIMITE_TIEMPO_TAG:
                modo_apriltag = False
                print(">>> TIEMPO AGOTADO - Volviendo a reconocer manos")
        
        # Ejecutar movimiento de Tag solo si estamos en este modo
        now = time.time()
    ...
    if not modo_apriltag:
        ...
        if etiqueta == "ARRIBA":
            ...
            lr, fb, ud, yaw = 0, 0, 0, 0
            time.sleep(2)
        elif etiqueta == "MANO ABIERTA" and volando==False:
            tello.takeoff()
            lr, fb, ud, yaw = 0, 0, 0, 0
            time.sleep(1)
            volando  = True
